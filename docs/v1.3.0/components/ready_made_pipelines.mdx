...
- The [Retriever](/components/v1.3.0/retriever) combs through a database and returns only the documents that it deems to be the most relevant to the query.
- The [Reader](/components/v1.3.0/reader) accepts the documents returned by the Retriever and selects a text span as the answer to the query.
...
The **output** of the pipeline is a Python dictionary with a list of [Answer](/components/v1.3.0/primitives#answer) objects stored under the `answers` key.
...
For more examples that showcase `ExtractiveQAPipeline`, check out one of our tutorials [here](/tutorials/v1.3.0/first-qa-system) or [here](/tutorials/v1.3.0/without-elasticsearch).
...
`DocumentSearchPipeline` wraps the [Retriever](/components/v1.3.0/retriever) into a pipeline. Note that this wrapper does not endow the Retrievers with additional functionality but instead allows them to be used consistently with other Haystack Pipeline objects and with the same familiar syntax.
...
In the pipeline's **output**, a list of [Document](/components/v1.3.0/primitives#document) objects is returned under the `document` key.
...
`GenerativeQAPipeline` combines the [Retriever](/components/v1.3.0/retriever) with the [Generator](/components/v1.3.0/generator). To create an answer, the Generator uses the internal factual knowledge stored in the language model’s parameters in addition to the external knowledge provided by the Retriever’s output.
...
For more examples on using `GenerativeQAPipeline`, check out our tutorials where we implement generative QA systems with [RAG](/tutorials/v1.3.0/retrieval-augmented-generation ) and [LFQA](/tutorials/v1.3.0/lfqa).
...
`SearchSummarizationPipeline` combines the [Retriever](/components/v1.3.0/retriever) with the [Summarizer](/components/v1.3.0/summarizer).
...
Under the `documents` key of the output, you will find a list of [Document](/components/v1.3.0/primitives#document) objects.
See the [DocumentSearchPipeline](/components/v1.3.0/ready-made-pipelines#documentsearchpipeline) on how to best print the output.
...
- The [Translator](/components/v1.3.0/translator), which translates a query source into a target language (e.g. German into English)
...
FAQPipeline wraps the [Retriever](/components/v1.3.0/retriever) into a pipeline and allows it to be used for question answering with FAQ data.
...
You will find a list of [Answer](/components/v1.3.0/primitives#answer) objects under the `answers` key of the pipeline output.
You see the [Document](/components/v1.3.0/primitives#document) objects from which the pipeline is getting the answer but looking at the `documents` key of the pipeline output.
...
Check out our [tutorial](/tutorials/v1.3.0/existing-faqs) for more information on FAQPipeline.
...
The output will be a list of [Document](/components/v1.3.0/primitives#document) objects.
