...
- The [Retriever](/v1.1.0/components/retriever) combs through a database and returns only the documents that it deems to be the most relevant to the query.
- The [Reader](/v1.1.0/components/reader) accepts the documents returned by the Retriever and selects a text span as the answer to the query.
...
The **output** of the pipeline is a Python dictionary with a list of [Answer](/v1.1.0/components/primitives#answer) objects stored under the `answers` key.
...
For more examples that showcase `ExtractiveQAPipeline`, check out one of our tutorials [here](/v1.1.0/tutorials/first-qa-system) or [here](/v1.1.0/tutorials/without-elasticsearch).
...
`DocumentSearchPipeline` wraps the [Retriever](/v1.1.0/components/retriever) into a pipeline. Note that this wrapper does not endow the Retrievers with additional functionality but instead allows them to be used consistently with other Haystack Pipeline objects and with the same familiar syntax.
...
In the pipeline's **output**, a list of [Document](/v1.1.0/components/primitives#document) objects is returned under the `document` key.
...
`GenerativeQAPipeline` combines the [Retriever](/v1.1.0/components/retriever) with the [Generator](/v1.1.0/components/generator). To create an answer, the Generator uses the internal factual knowledge stored in the language model’s parameters in addition to the external knowledge provided by the Retriever’s output.
...
For more examples on using `GenerativeQAPipeline`, check out our tutorials where we implement generative QA systems with [RAG](/v1.1.0/tutorials/retrieval-augmented-generation ) and [LFQA](/v1.1.0/tutorials/lfqa).
...
`SearchSummarizationPipeline` combines the [Retriever](/v1.1.0/components/retriever) with the [Summarizer](/v1.1.0/components/summarizer).
...
Under the `documents` key of the output, you will find a list of [Document](/v1.1.0/components/primitives#document) objects.
See the [DocumentSearchPipeline](/v1.1.0/components/ready-made-pipelines#documentsearchpipeline) on how to best print the output.
...
- The [Translator](/v1.1.0/components/translator), which translates a query source into a target language (e.g. German into English)
...
FAQPipeline wraps the [Retriever](/v1.1.0/components/retriever) into a pipeline and allows it to be used for question answering with FAQ data.
...
You will find a list of [Answer](/v1.1.0/components/primitives#answer) objects under the `answers` key of the pipeline output.
You see the [Document](/v1.1.0/components/primitives#document) objects from which the pipeline is getting the answer but looking at the `documents` key of the pipeline output.
...
Check out our [tutorial](/v1.1.0/tutorials/existing-faqs) for more information on FAQPipeline.
...
The output will be a list of [Document](/v1.1.0/components/primitives#document) objects.
