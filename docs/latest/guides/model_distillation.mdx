...
The goal of model distillation is for the student model to behave similarly to the teacher model. This similarity can be enforced using a range of different loss functions. There are various approaches to model distillation using different sets of loss functions when training the student. The two approaches we have focussed on are the logits-based approach proposed by [Hinton et al.](https://arxiv.org/pdf/1503.02531.pdf) and the approach used by [TinyBERT (Jiao et al.)](https://arxiv.org/pdf/1909.10351).

The logits-based approach introduces a loss function to minimise the difference between the logits of the student model and the teacher model. These logits contain the probabilities that the teacher model assigns to all possible response spans in the document. This makes the training loss more expressive and allows for higher accuracy with the same model size. An advantage of this method is that it is very quick and does not have any requirements for the student architecture.

The TinyBERT approach consists of two steps: In the first step, the differences between the hidden states and the attentions of the student of student and teacher are minimised. In the second step, the output logits are compared similarly to the logits-based approach. An advantage of this method is that it delivers much better results than just applying to logits-based approach. However, compared to the logits-based approach this approach takes much longer (a few days instead of a few hours).
...
### Data augmentation (for using TinyBERT augmentation)

The first stage of the TinyBERT approach requires a lot of data. Because acquiring a lot of training data is quite expensive, the TinyBERT paper suggests a data augmentation approach to multiply existing datasets. To use data augmentation in haystack, you can use the [`augment_squad.py` script](https://github.com/deepset-ai/haystack/blob/master/haystack/utils/augment_squad.py) provided in haystack. You could use the following command if the dataset is called `dataset.json` and is in the same folder as `augment_squad.py`:
```
python augment_squad.py --squad_path dataset.json --ouput_path augmented_dataset.json --multiplication_factor 20
```
This would create an augmented dataset with 20 times the samples in the file `augmented_dataset.json`.
...
#### Logits-based approach
For using logits-based distillation in Haystack, you just need to call the `distil_from` method on the student and pass the teacher model. 
...
#### TinyBERT
To use TinyBERT distillation in haystack, you also just need to call the `distil_from` method on the student and pass the teacher model. However, you need to provide a few additional parameters:

```
student.distil_from(teacher, tinybert_loss=True,
                    data_dir=data_dir, train_filename=train_filename,
                    tinybert_train_filename=augmented_dataset,
                    tinybert_epochs=5, tinybert_learning_rate=5e-5 ...)
```
To activate the additional distillation step for TinyBERT distillation, you need to set the `tinybert_loss` parameter to `True` and you should also provide an augmented dataset (see above for instructions on how to use the augmentation script) for this first stage. This dataset must be specified using the `tinybert_train_filename` parameter.

If you want, you can also specify the learning rate and the number of epochs for the first TinyBERT distillation step. Otherwise, they will be set to recommended values.

## What parameters should I use?
### Logits-based approach
Logits-based model distillation is controlled by two parameters: `temperature` and `distillation_loss_weight`.
...
### TinyBERT
With TinyBERT, we would recommend just using the default parameters.
...
| Student | Logits | TinyBERT | Base F1 | Logits F1 | TinyBERT F1 | Teacher F1 | Base F1 | Large F1 |
| huawei-noah/TinyBERT_General_6L_768D | | | 71.1% | 73.0% | 76.2 % | 77.8% | 83.2% |
