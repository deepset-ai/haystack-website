...
For this reason, choosing the right reader is always a trade-off between quality of results and deployment cost. There are different approaches to bridge the performance gap between large language models and smaller ones.
...
`distillation_loss_weight` specifies the weight given to the distillation loss in relation to the loss based on the label. For example, setting this to 0 would effectively disable model distillation
...
| Student model | Teacher model | Dataset | `temperature` | `distillation_loss_weight` | Distilled model EM | Distilled model F1 | Baseline EM | Baseline F1 | Teacher EM | Teacher F1 |
|-|-|-|-|-|-|-|-|-|-|-|
| prajjwal1/bert-medium | deepset/bert-large-uncased-whole-word-masking-squad2 | SQuADv2 | 5.0 | 1.0 | 68.6% | 72.8% | 65.6% | 69.6% | 78.9% | 83.2%7 
| roberta-base | deepset/roberta-large-squad2 | SQuADv2 | 1.5 | 0.75 | 79.8% | 83.9% | 78.3% | 82.6% | 83.6% | 87.9% | 73.8% | 
| deepset/xlm-roberta-base | deepset/xlm-roberta-large-squad2 | SQuADv2 | 3.0 | 0.75 | 74.8% | 79.0% | 73.8% | 78% | 80.0% | 84.4% |
| deepset/gelectra-base | deepset/gelectra-large-germanquad | GermanQuAD | 2.0 | 0.75 | 61.8% | 80.4% | 59.4% | 77.2% | 68.0% | 86.4% |
