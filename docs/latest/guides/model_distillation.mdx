...
`distil_from` accepts all parameters that you can use for training, but also accepts additional parameters. The only parameter necessary is the teacher model, but we would also recommend adjusting `temperature` and `distillation_loss_weight`.
...
In most experiments, we found a temperature of 5 to be most useful.
