...
When using model distillation, you need to have both a teacher and a student model. The teacher model must be trained on the task. This means that you can use a fine-tuned teacher from the huggingface hub or you can fine-tune the teacher on your own data beforehand.
