# Preprocessing
This guide shows you how to get from a raw file to a cleaned `Document` that is ready to be indexed in a `DocumentStore`
...
You can index `Document` objects or plain dictionaries to a Haystack `DocumentStore`. Therefore, the output of your preprocessing should comply to one of these two formats (we recommend `Document`, see [Primitives](/components/primitives) for more details): 
