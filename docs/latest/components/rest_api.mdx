...
By default, the Haystack API container will start with a pipeline defined in the [ haystack/rest_api/rest_api/pipeline/pipelines.haystack-pipeline.yml ](https://github.com/deepset-ai/haystack/blob/main/rest_api/rest_api/pipeline/pipelines.haystack-pipeline.yml) file. If you want to direct the API container to use a YAML file at a different location, you can set the [`PIPELINE_YAML_PATH` environment variable](https://github.com/deepset-ai/haystack/blob/master/rest_api/config.py#L3).
...
See the definitions of the different endpoints available on the latest version [here](/components/rest-api-definition).
