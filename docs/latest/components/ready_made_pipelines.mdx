...
- The [Retriever](/pipeline_nodes/retriever) combs through a database and returns only the documents that it deems to be the most relevant to the query.
- The [Reader](/pipeline_nodes/reader) accepts the documents returned by the Retriever and selects a text span as the answer to the query.
...
The **output** of the pipeline is a Python dictionary with a list of [Answer](/components/documents-answers-labels#answer) objects stored under the `answers` key.
...
`DocumentSearchPipeline` wraps the [Retriever](/pipeline_nodes/retriever) into a pipeline. Note that this wrapper does not endow the Retrievers with additional functionality but instead allows them to be used consistently with other Haystack Pipeline objects and with the same familiar syntax.
...
`GenerativeQAPipeline` combines the [Retriever](/pipeline_nodes/retriever) with the [Generator](/pipeline_nodes/generator). To create an answer, the Generator uses the internal factual knowledge stored in the language model’s parameters in addition to the external knowledge provided by the Retriever’s output.
...
`SearchSummarizationPipeline` combines the [Retriever](/pipeline_nodes/retriever) with the [Summarizer](/pipeline_nodes/summarizer).
...
Under the `documents` key of the output, you will find a list of [Document](/components/documents-answers-labels#document) objects.
...
FAQPipeline wraps the [Retriever](/pipeline_nodes/retriever) into a pipeline and allows it to be used for question answering with FAQ data.
...
You see the [Document](/components/documents-answers-labels#document) objects from which the pipeline is getting the answer but looking at the `documents` key of the pipeline output.
