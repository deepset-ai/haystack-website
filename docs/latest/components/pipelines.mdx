...
Thanks to the modularity of pipelines, you can create your own nodes and comfortably integrate them into your system. There are a few easy steps to create a custom node:
...
- Define a `run()` method that will be executed when you node is called a the Pipeline. The input arguments should consist of all "config" params that you want to allow plus those "data" arguments that you will expect as input from a previous node (e.g. `documents`, `query`, `file_paths` ...). 
- Let `run()` return a tuple, where the first element is a dict that contains the data you want to pass to the next node and the second is the name of the outgoing edge (usually `"output_1"`)
- Optional: Add any custom debug information in `output["_debug"]`. This will be accessible in the pipeline output when the debug mode is enabled.  
