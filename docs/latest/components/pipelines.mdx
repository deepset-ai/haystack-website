...
## Indexing Pipelines vs Querying Pipelines

Indexing pipelines prepare your files for search.
Their main objective is to convert your files into Haystack [Documents](/components/documents-answers-labels#document),
so that they can be saved in a [Document Store](/components/document-store).

They begin with either a [File Converter](/pipeline_nodes/file-converters),
or a [File Classifier](/pipeline_nodes/file-classifier) followed by File Converters.

When Nodes are added into the middle of an indexing pipeline,
they can also perform extra steps of processing on Documents before they are saved into the Document Store.
For example, a [PreProcessor](/pipeline_nodes/preprocessor) can be added to perform Document splitting and cleaning.
A [Document Classifier](/pipeline_nodes/document-classifier) in an indexing pipeline attaches a classification label to each Document.
...
**Tutorial:** For a more detailed example of this kind of pipeline, have a look at the [Document Classification at Indexing Tutorial](/tutorials/doc-class-index).

</div>

Querying pipelines perform searches (or any other NLP task) on a set of documents that have been indexed into a Document Store.
The first Node in a querying pipeline will take "Query" as its input and the documents will most often be found within the Retriever which in turn contains a Document Store.
A querying pipeline for Open-Domain QA might look something like this:
...
For query pipelines, you will need to provide `query` as an argument:
``` python
