...
By default, a new pipeline receives a root node called `Query` or `File` depending on if it's a Query or Indexing Pipeline, which is the entry point to the pipeline graph. From that point on, you need to manually define how the information flows from one node to the next.
