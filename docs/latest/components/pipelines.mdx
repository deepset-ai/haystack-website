...
Indexing pipelines take files as input. They convert them into Haystack Documents and perform some processing steps. Then they index the Documents into a Document Store.
The first Node in an indexing pipeline will usually be a [File Converter](/pipeline_nodes/file_converters) or a FileTypeClassifier and take "File" as its input.
The last Node will always be a Document Store.
...
**Tutorial:** have a look at the [Document Classification at Indexing Tutorial](/tutorials/doc-class-index) for a more detailed example of this kind of pipeline.
