...
def run(self, query: str):
    if "?" in query:
        return {"_debug": {"custom": "My debug info 1"}, "output_1"
    else:
        return {"_debug": {"custom": "My debug info 2"}, "output_2"
 ```

This `_debug` gets appended to a "global" `_debug` dict storing per Node debug data that gets returned with the final output.
...
```
{
    "answers": ...,
    "_debug": {
        "node_a": {"custom": "My debug info 1"},
        "node_b": {"custom": "My debug info 2"}
    }
}
```
