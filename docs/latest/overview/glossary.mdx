...
**File** - A raw file that you upload to Haystack, for example a PDF file. When an indexing pipeline runs, filest get converted, cleaned, and split into *Documents*, which contain the actual text and are then used for finding the best answer to a query.
...
**Query** - In NLP, a query is a question that uses natural, spoken language without any special syntax or format. 