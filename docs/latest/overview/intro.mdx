...
- **NLP for Search**: Pick components that perform [retrieval](/components/retriever),
[question answering](/components/reader), [reranking](/components/ranker) and much more
...
- **Flexible databases**: Load data into and query from a range of [databases](/components/document_store) such as Elasticsearch, Milvus, FAISS, SQL and more

- **Scalability**: [Scale your system](/guides/optimization) to handle millions of documents and deploy them via [REST API](/guides/rest-api)

- **Domain adaptation**: All tooling you need to [annotate](/guides/annotation) examples, collect [user-feedback](/guides/domain-adaptation#user-feedback), [evaluate](/guides/evaluation) components and [finetune](/guides/domain-adaptation) models.
...
but Machine Learning has enabled a new style of search based on word meaning rather than string matching.
As new language processing models are developed, new styles of search are also possible.
In Haystack, you can start systems to perform:
...
These are just a few examples of what can be created in Haystack.
...
Haystack offers different [Nodes](/components/nodes) that perform different kinds of text processing.
...
For example, if you would like to [perform question answering with a Reader model](http://localhost:3000/components/reader#usage),
...
On top of simplifying data flow logic, this also allows for complex routing options such as those involving [decision nodes](/components/nodes/decision-nodes).
...
For this, Haystack comes with a [REST API](/guides/rest-api) that is designed to work in production environments.

When set up like this, you can save and load [Pipelines from YAML files](/components/pipelines#yaml-file-definitions),
interact with Pipelines via HTTP requests and connect Haystack to a user-facing GUI.
...
harnesses the reading comprehension power of the [Reader](/components/reader)
but also applies it to large document bases with the help of the [Retriever](/components/retriever).
