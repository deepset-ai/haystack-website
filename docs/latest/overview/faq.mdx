...
on our [Github page](https://github.com/deepset-ai/haystack/blob/main/README.md#7-rest-api).
...
## Why is Haystack not logging everything to the console?

The default logging level in Haystack is set to WARNING.
This means that messages on the INFO level will not be logged.
For example, if you call FARMReader.train() the evaluation reports won't be shown in the console output by default.
In order to see logs at INFO level, run the following before your first import of Haystack:
