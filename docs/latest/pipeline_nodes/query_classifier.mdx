...
To use the QueryClassifier within a pipeline as a [decision node](/docs/pipeline_nodes/overview#decision-nodes), run:
...
## Distinguishing between Keywords, Questions and Statements

One recommended setup would be to route both questions and statements to a DensePassageRetriever, which is better accustomed to these types, while routing keyword queries to an ElasticsearchRetriever.
To learn about these different types, see [Query Type Definitions](/docs/pipeline_nodes/query)

![image](https://user-images.githubusercontent.com/6007894/127831511-f55bad86-4b4f-4b54-9889-7bba37e475c6.png)

A code example of this configuration is shown in [Usage](/docs/pipeline_nodes/query-classifier#usage).
...
| Name                                                                                                                            | Class                       | Initialization                                                                                      | Output 1           | Output 2 |
|---------------------------------------------------------------------------------------------------------------------------------|-----------------------------|-----------------------------------------------------------------------------------------------------|--------------------|----------|
| [`shahrukhx01/bert-mini-finetune-question-detection`](https://huggingface.co/shahrukhx01/bert-mini-finetune-question-detection) | TransformersQueryClassifier | TransformersQueryClassifier(model_name_or_path="shahrukhx01/bert-mini-finetune-question-detection") | Question/Statement | Keyword  |
|                                                                                                                                 |                             |                                                                                                     |                    |          |
|                                                                                                                                 |                             |                                                                                                     |                    |          |
...
Usage (Transformers with Custom model)
- standalone
- pipeline

Keywords, Questions, Statements
The most common form of query classification is to distinguish between
