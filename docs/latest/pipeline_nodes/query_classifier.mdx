...
By passing on queries to Nodes that are more suited to that kind of query, you can expect to get better search results.
For example, the Dense Passage Retriever is trained on full questions and so it makes sense to only pass questions to it.
...
|__Input__       | Query                                                                                                                                                                  |
|__Output__      | Query                                                                                                                                                                    |
|__Classes__     | TransformersQueryClassifier, SklearnQueryClassifier                                                                                                                                             |
|||
...
The Query Classifier can be used as a standalone node:
...
Query Classifier models should be able to make its prediction separate from the presence or absence of the question mark.
...
These queries consist also of a regular, grammatical sentences.
However, they are declarative in nature rather than interrogative:
...
Pass your own `Transformer` binary classification model from file or use one of the following pretrained models hosted on Huggingface:
