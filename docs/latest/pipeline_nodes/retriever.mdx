...
The Retriever can perform Document Retrieval by sweeping through a document store and returning a set of candidate documents that are relevant to the query.
...
The Retriever is tightly coupled with the Document Store and requires a Document Store object in its initialization.
While the Retriever functions as a Node in the Pipeline, the Document Store does not.
When it is run, the Retriever receives a Query as input, which it uses to query into the documents contained in the Document Store,
before passing on the best candidates.
...
|__Classes__     | BM25Retriever, ElasticsearchRetriever, DensePassageRetriever, TableTextRetriever, EmbeddingRetriever, TfidfRetriever, ElasticsearchFilterOnlyRetriever                                                                                                           |
|||

Our starting recommendations would be to use an Embedding Retriever if GPU acceleration is available,
and a BM25 based sparse retriever like the ElasticsearchRetriever if it is not.
Have a read of the sections below to learn about different types of Retrievers.
...
BM25 is a variant of [TF-IDF](https://haystack.deepset.ai/pipeline_nodes/retriever#tf-idf) that we recommend you use if you are looking for a retrieval method that does not need a neural network for indexing.
