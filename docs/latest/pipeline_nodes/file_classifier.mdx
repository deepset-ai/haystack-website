...
**Note: ** The FileTypeClassifier works best when you pass in one file per `Pipeline.run()` call.
    If multiple files are passed in to `Pipeline.run()` or `Pipeline.run_batch()`, the FileTypeClassifier currently throws an error if they are not of the same type.
