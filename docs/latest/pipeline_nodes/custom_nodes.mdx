...
2. If your node's output will be routed to a fixed number of nodes, set `outgoing_edges` as a class attribute. Most node have one outgoing edge. Decision nodes have more than one outgoing edge. If your node has a variable number of outgoing edges, define `_calculate_outgoing_edges()`. See the [`FileClassifier` class](https://github.com/deepset-ai/haystack/blob/862ac31b5c4349519cc2c472bca5bfcee1944c95/haystack/nodes/file_classifier/file_type.py#L54) for an example.
