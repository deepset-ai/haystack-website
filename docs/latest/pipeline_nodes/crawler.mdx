...
The Crawler scrapes the text from a website, creates a Haystack Document object out of it, and saves it to a file. 
...
docs = crawler.crawl(
    ...
    crawler_depth=1 #this tells the Crawler to follow only the links that it finds on the initial URL
)
```
