...
They are particularly useful when your Retriever has high recall but poor relevance scoring.
...
In Haystack, you can use any [Cross-Encoder](https://huggingface.co/cross-encoder) model that returns as a single logit
...
While the BM25 retriever is fast and light weight, it is not sensitive to word order but rather treats text as a bag of words.
