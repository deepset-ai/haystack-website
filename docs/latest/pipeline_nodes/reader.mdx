...
This can be particularly useful if want to upload the model to the Hugging Face Model Hub.
...
To convert a language model into a Reader model, you must firts train it on a Question Answering dataset.
To do so, you must add a question answering prediction head on top of the language model.
You can think of it as a token classification task where every input token is assigned a probability of being
