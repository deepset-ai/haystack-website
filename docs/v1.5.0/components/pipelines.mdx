...
To build modern search pipelines, you need two things: powerful [building blocks](/pipeline_nodes/v1.5.0/overview) and an easy way to stick them together.
...
```
For query pipelines, there's an additional function `run_batch()` that you can use to provide multiple queries as input:
``` python
