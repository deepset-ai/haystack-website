...
By far the most common way to use a Document Store in Haystack is to
fetch documents using a Retriever.
A Document Store needs to be provided as an argument to the initialization of a Retriever.
Note that the Retriever functions as a [Node](/pipeline_nodes/v1.4.0/overview)
while a Document Store does not.
    ...
    options={[
        {
            ...
            content: (
                <div>
                <a href="https://www.elastic.co/guide/en/elasticsearch/reference/v1.4.0/current/install-elasticsearch.html">Install</a>&nbsp;
                Elasticsearch and then <a href="https://www.elastic.co/guide/en/elasticsearch/reference/v1.4.0/current/starting-elasticsearch.html">start</a>&nbsp;
        ...
        },
        {
            ...
            content: (
                <div>
                To initialize the Pinecone document store, provide an API key and Pinecone cloud environment. To obtain the API key, <a href="https://app.pinecone.io">create a free Pinecone account</a>. The enviroment is <b>us-west1-gcp</b> by default. 
               
                <pre>
                    <code>import os</code>
                    <code>from haystack.document_store import PineconeDocumentStore</code>
...
See [PreProcessor}](/pipeline_nodes/v1.4.0/preprocessor) for more information on the cleaning and splitting steps that will help you maximize Haystack's performance.
    ...
    options={[
        ...
        },
        {
            ...
            content: (
                ...
                <strong>Pros:</strong>
                <ul>
                    <li>A fully managed service for large-scale dense retrieval</li>
                    <li>Low query latency at any scale</li>
                    <li>Live index updates</li>
                    <li>Stores embeddings and metadata separately from the document content which makes it easier to setup infrastructure and maintenance</li>
                </ul>
                <strong>Cons:</strong>
                <ul>
                    <li>You still need to run a SQL database locally</li>
                    <li>You need to be on a paid plan to fully benefit from it</li>
                </ul>
                </div>
            )
        }
...
you can very quickly make a Haystack compliant version using our `elasticsearch_index_to_document_store` or `open_search_index_to_document_store` function.
